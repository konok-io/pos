...
function MainApp({ currentUser, onLogout }) {
  ...
  const updateSuppliers = async (newSuppliers) => {
    try {
      const currentIds = new Set(suppliers.map(s => s.id));
      const newIds = new Set(newSuppliers.map(s => s.id));
      
      // Find deleted items
      const toDelete = suppliers.filter(s => !newIds.has(s.id));
      ...
      // Find added items
      const toAdd = newSuppliers.filter(s => !currentIds.has(s.id));
      ...
      // Find updated items
      const toUpdate = newSuppliers.filter(ns => {
        const os = suppliers.find(s => s.id === ns.id);
        return os && JSON.stringify(os) !== JSON.stringify(ns);
      });
      for (const s of toUpdate) {
        try {
          await suppliersApi.update(s);
        } catch (err) {
  ...
  const upd = {
    ...
    // Suppliers - now a function that accepts array
    suppliers: async (newSuppliers) => {
      ...
      <div style={{background:T.white,padding:'0 24px',flexShrink:0,boxShadow:'0 4px 20px rgba(0,0,0,0.08)',borderBottom:`2px solid ${T.teal}`}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',height:64}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:14,flexShrink:0}}>
            <div className="header-logo-icon" style={{width:60,height:60,background:'linear-gradient(135deg, #0F766E 0%, #115E59 100%)',borderRadius:14,display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 4px 12px rgba(15,118,110,0.3)',fontSize:32}}>🏪</div>
            <div>
              <div className="header-business-name" style={{fontWeight:800,fontSize:24,color:T.gray900,lineHeight:1.2}}>{settings.name}</div>
              <div style={{fontSize:15,color:T.gray400}}>POS ম্যানেজমেন্ট সিস্টেম</div>
            </div>
          </div>
...
function ProductsScreen({products, suppliers, categories, purchases, productHistory, upd, settings}) {
  ...
  const del = async (id) => {
    ...
    // Save product deletion to history BEFORE deleting
    const deletionEntry = {
      id: genId(),
    ...
    // Delete stock/price history for this product BUT keep deletion entry
    await upd.productHistory([...productHistory.filter(h=>h.productId!==id), deletionEntry]);
  };
...
# পণ্য ডেমো (পণ্য যুক্ত করার সময় উপরের সরবরাহকারী ও ক্যাটাগরি ডেমো আগে ইমপোর্ট করুন)
...
function SuppliersScreen({suppliers, products, categories, purchases, upd, refreshData}) {
  ...
  const handleAddSupplier = async () => {
    ...
    const nameLower = newSupplier.name.trim().toLowerCase();
    ...
    let codeToUse = newSupplier.code?.trim();
    if (!codeToUse) {
      // Find max code and generate next
      ...
      const codeExists = suppliers.some(s => s.code === codeToUse);
      if (codeExists) {
        alert('❌ এই কোম্পানি আইডি ইতিমধ্যে আছে!');
        return;
      }
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setNewSupplier({name:'',phone:'',email:'',address:'',company:'',crNumber:'',vatNumber:'',code:''});
        // Refresh data
        await refreshData();
      } else {
  ...
  // Filter and sort by products count (highest first)
  ...
  const saveCompany = async () => {
    ...
    // Check if edit mode (modal.mode comes from the modal component)
    const isEditMode = modal?.mode === 'edit';
    
    if (isEditMode) {
      // Edit mode - UPDATE existing supplier (always)
      const currentId = modal.id;
      const originalName = modal.originalName || '';
      const originalNameLower = originalName.toLowerCase();
      
      // Check if name is being changed to a different name
      const isNameChanged = newNameLower !== originalNameLower;
      
      if (isNameChanged) {
        // Name changed - check if new name already exists (excluding current)
        const exists = suppliers.some(s => 
          s.id !== currentId && (s.name||'').toLowerCase().trim() === newNameLower
        );
        if (exists) {
          alert('❌ এই সরবরাহকারীর নাম ইতিমধ্যে আছে!'); 
          return; 
      ...
      const currentSupplier = suppliers.find(s => s.id === currentId);
      
      // UPDATE existing supplier
      const updatedSupplier = { 
        ...currentSupplier,
        name: newName, 
        phone: form.phone || currentSupplier?.phone || '', 
        address: form.address || currentSupplier?.address || '', 
        vatNumber: form.vatNumber || currentSupplier?.vatNumber || '', 
        crNumber: form.crNumber || currentSupplier?.crNumber || '',
        isAuto: false
      };
      
      const updatedList = suppliers.map(s => s.id === currentId ? updatedSupplier : s);
      await upd.suppliers(updatedList);
      setModal(null);
      setForm({});
      if (isNameChanged) {
      ...
      // Check for duplicate name
      const exists = suppliers.some(s => 
        (s.name||'').toLowerCase().trim() === newNameLower
      );
      if (exists) {
        alert('❌ এই সরবরাহকারীর নাম ইতিমধ্যে আছে!'); 
        return; 
      ...
      const newS = { id: genId(), code: newCode, name: newName, phone: form.phone||'', address: form.address||'', vatNumber: form.vatNumber||'', crNumber: form.crNumber||'' };
  ...
  const saveCategory = async () => {
    ...
    if (modal?.mode === 'editCat') {
      const oldName = modal.catName;
      const newName = catForm.name.trim();
      
      // Check if products exist in this category - prevent name change
      const hasProducts = products.some(p => (p.cat||'').toLowerCase() === oldName.toLowerCase());
      if (hasProducts && oldName.toLowerCase() !== newName.toLowerCase()) {
        alert('❌ এই ক্যাটাগরিতে পণ্য আছে বলে নাম পরিবর্তন করা যাবে না!');
        return;
      }
      
      const exists = categories.some(c => (c.name||'').toLowerCase().trim() === newName.toLowerCase() && c.name !== oldName);
      if (exists) { alert('❌ এই ক্যাটাগরির নাম ইতিমধ্যে আছে!'); return; }
      
      // Check if products exist before allowing edit
      if (hasProducts) {
        alert('❌ এই ক্যাটাগরিতে পণ্য আছে বলে ইডিট করা যাবে না!');
        return;
      }
      
      const updatedCategories = categories.map(c => c.name === oldName ? {...c, name: newName} : c);
      await upd.categories(updatedCategories);
      const updatedProducts = products.map(p => p.cat === oldName ? {...p, cat: newName} : p);
      await upd.products(updatedProducts);
      alert(`✅ ক্যাটাগরি আপডেট করা হয়েছে!\n${oldName} → ${newName}`);
      setModal(null);
      setCatForm({name:''});
      if (viewCategory) setViewCategory({...viewCategory, name: newName});
    } else {
      const exists = categories.some(c => (c.name||'').toLowerCase().trim() === (catForm.name||'').toLowerCase().trim());
      if (exists) { alert('❌ এই ক্যাটাগরির নাম ইতিমধ্যে আছে!'); return; }
      const newCat = { id: genId(), name: catForm.name.trim() };
...
সৌন্দর্য`;
  ...
  const del = async (id) => {
    ...
    // Check if this company has products
  ...
  const handleSuppliersCsvImport = async (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        // Parse CSV header (remove BOM if present)
        const headers = lines[0].split(',').map(h => h.trim().replace(/"/g, '').replace(/^\uFEFF/, ''));
        ...
        // Process data - ONLY suppliers (NO categories)
        const errors = [];
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          // Get supplier info - ONLY supplier related fields (no categories)
          ...
          // Check for duplicate in existing suppliers
          const existsInDB = suppliers.some(s => (s.name||'').toLowerCase() === csvNameLower);
          if (existsInDB) {
            errors.push(`সারি ${rowNum}: সরবরাহকারী "${csvName}" ডাটাবেজে ইতিমধ্যে আছে`);
            continue;
          }
          
          // Check for duplicate within same CSV
          if (csvSupplierNames.has(csvNameLower)) {
            errors.push(`সারি ${rowNum}: সরবরাহকারী "${csvName}" CSV ফাইলে ডুপ্লিকেট`);
            continue;
          }
          ...
          const newCode = csvCode || `C-${String(maxCode + suppliersToAdd.length + 1).padStart(5, '0')}`;
          
          // Check for duplicate code
          if (csvCode && suppliers.some(s => s.code === csvCode)) {
          ...
          csvSupplierNames.add(csvNameLower);
        }
        
        // Save changes - ONLY suppliers
        const suppliersAdded = suppliersToAdd.length;
        
        // Add each new supplier via API
        for (const supplier of suppliersToAdd) {
          try {
            await suppliersApi.create(supplier);
          } catch (e) {
            errors.push(`সরবরাহকারী যুক্ত করতে ব্যর্থ: ${supplier.name}`);
          }
        }
        ...
        msg += `🏢 নতুন সরবরাহকারী: ${suppliersAdded}টি\n`;
        if (errors.length > 0) {
      ...
      <div style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden',background:T.gray50}}>
        ...
        <div style={{padding:'16px 20px',display:'flex',gap:12,alignItems:'center',background:T.white,borderBottom:`1px solid ${T.gray200}`}}>
          ...
          <div style={{marginLeft:'auto',display:'flex',gap:8}}>
            ...
            {!viewSupplier.isAuto && supProducts.length === 0 && (
              <>
                <button style={{...btn(),margin:'0 2px'}} onClick={()=>{setViewSupplier(null);setForm({...viewSupplier});setModal({mode:'edit',id:viewSupplier.id,originalName:viewSupplier.name});}}>✏️</button>
                <button style={{...btn('danger'),margin:'0 2px'}} onClick={()=>{setViewSupplier(null);del(viewSupplier.id);}}>🗑️</button>
              </>
            )}
            {!viewSupplier.isAuto && supProducts.length > 0 && (
              <span style={{fontSize:13,color:T.gray500,padding:'6px 12px'}}>পণ্য আছে বলে ইডিট/ডিলিট করা যাবে না</span>
            )}
          ...
          <div>
            ...
            <div style={{...card,display:'flex',flexDirection:'column',maxHeight:'calc(100vh - 420px)'}}>
              <table style={{width:'100%',borderCollapse:'collapse',background:T.white,tableLayout:'fixed'}}>
                <thead>
                  <tr style={{background:T.tealLight}}>
                    {['ক্রম','আইডি','সরবরাহকারীর নাম','ঠিকানা','CR নম্বর','VAT নম্বর','পণ্য','একশন'].map((h,i)=>(
                      <th key={i} style={{padding:'10px 8px',textAlign:i===7?'center':'left',fontSize:14,fontWeight:700,color:T.teal,letterSpacing:'0.3px',whiteSpace:'nowrap'}}>{h}</th>
                    ))}
                  </tr>
                </thead>
              </table>
              <div style={{flex:1,overflow:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',background:T.white,tableLayout:'fixed'}}>
                  <tbody>
                    {filtered.length === 0 ? (
                      <tr><td colSpan={8} style={{padding:40,textAlign:'center',color:T.gray400}}>কোনো সরবরাহকারী পাওয়া যায়নি</td></tr>
                    ) : filtered.map((s,i)=>{
                      const hasProducts = products.some(p => (p.company||'').toLowerCase() === (s.name||'').toLowerCase());
                      return (
                      <tr key={s.id} style={{background:i%2===0?T.white:'#FAFAFA',borderBottom:`1px solid ${T.gray100}`}}>
                        ...
                        <td style={{padding:'10px 8px',fontSize:13,fontWeight:600,color:T.gray600,fontFamily:'monospace'}}>{s.code||'-'}</td>
                        ...
                        <td style={{padding:'10px 8px',whiteSpace:'nowrap',textAlign:'center'}}>
                          <button onClick={()=>setViewSupplier(s)} style={{...btn(),fontSize:14,padding:'4px 8px',margin:'0 2px'}}>👁️</button>
                          {!s.isAuto && !hasProducts && (
                            <>
                              <button onClick={e=>{e.stopPropagation();setForm({...s});setModal({mode:'edit',id:s.id,originalName:s.name});}} style={{...btn('primary'),padding:'4px 6px',margin:'0 2px'}}>✏️</button>
                              <button onClick={e=>{e.stopPropagation();del(s.id);}} style={{...btn('danger'),padding:'4px 6px',margin:'0 2px'}}>🗑️</button>
                            </>
                          )}
                          {!s.isAuto && hasProducts && (
                            <span style={{fontSize:12,color:T.gray400}}>পণ্য আছে</span>
                          )}
                        </td>
                      </tr>
                      );
              ...
              <>
                ...
                {modal.mode === 'add' && (
                  <div style={{marginBottom:12}}>
                    ...
                    <input value={form.code||''} onChange={e=>setForm(f=>({...f,code:e.target.value}))} placeholder={`যেমন: C-${String(suppliers.reduce((max,s)=>{const m=s.code?.match(/C-(\d+)/);return m?Math.max(max,parseInt(m[1])):max;},0)+1).padStart(5,'0')}`} style={input} />
                  </div>
                )}
                {modal.mode === 'edit' && form.code && (
                  <div style={{marginBottom:12,padding:'8px 12px',background:T.gray50,borderRadius:6,border:`1px solid ${T.gray200}`}}>
                    <label style={{...label,color:T.gray600}}>🔢 কোম্পানি আইডি</label>
                    <div style={{fontSize:16,fontWeight:600,fontFamily:'monospace',color:T.teal}}>{form.code}</div>
                  </div>
                )}
                <div style={{marginBottom:12}}>
          ...
          {filtered.map(c=>(
            <div key={c.id} style={{...card,cursor:'pointer',transition:'box-shadow 0.15s'}} onClick={()=>setViewId(c.id)}>
              ...
              <div style={{display:'flex',gap:6}} onClick={e=>e.stopPropagation()}>
                ...
                <button style={btn('primary','sm')} onClick={e=>{e.stopPropagation();setForm({...c});setModal({mode:'edit',id:c.id});}}>✏️</button>
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setExpensesList(Array.isArray(expensesData?.data) ? expensesData.data : []);
        setIncomesList(Array.isArray(incomesData?.data) ? incomesData.data : []);
      } catch (error) {
...
<div class="section">
  ...
  <table>
    <thead><tr><th style="text-align:center">আইডি</th><th>তারিখ</th><th>গ্রাহক</th><th>পরিমাণ</th></tr></thead>
    <tbody>
...
<div class="section">
  ...
  <table>
    <thead><tr><th style="text-align:center">আইডি</th><th>তারিখ</th><th>সরবরাহকারী</th><th>পরিমাণ</th></tr></thead>
    <tbody>
...
=========================================== */
function SettingsScreen({settings, products, suppliers, categories, purchases, sales, customers, productHistory, upd, currentUser, refreshData}) {
  const [form, setForm] = useState({
    name: '',
    address: '',
    phone: '',
  ...
  // Load users from API (server filters out super_admin)
  useEffect(() => {
                  ...
                  <div style={{ background: '#f1f5f9', borderRadius: 12, padding: 16 }}>
                    ...
                    <div style={{ background: '#fff', padding: `${form.receiptFontSize||11}px`, width: 220, margin: '0 auto', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', fontSize: `${form.receiptFontSize||11}px`, fontFamily: "'Tiro Bangla','Courier New',monospace", color: '#000' }}>
                      <div style={{ textAlign: 'center', borderBottom: '1px dashed #000', paddingBottom: 6, marginBottom: 6 }}>
                        ...
                        <div style={{ fontSize: `${(form.receiptFontSize||11)-1}px`, marginTop: 4 }}>Invoice ID: 12345678</div>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: `${(form.receiptFontSize||11)-1}px` }}>
                        ...
                        <tbody>
                          <tr><td style={{ padding: '3px 0' }}>পণ্য নাম ১<br/><span style={{ fontSize: `${(form.receiptFontSize||11)-3}px`, color: '#666' }}>কোম্পানি</span></td><td style={{ textAlign: 'center', padding: '3px 0' }}>2</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳50</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳100</td></tr>
                          <tr><td style={{ padding: '3px 0' }}>পণ্য নাম ২</td><td style={{ textAlign: 'center', padding: '3px 0' }}>1</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳75</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳75</td></tr>
                        </tbody>
                      </table>
                      <div style={{ borderTop: '1px dashed #000', marginTop: 6, paddingTop: 6 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: `${(form.receiptFontSize||11)-1}px` }}><span>সাবটোটাল:</span><span>৳175</span></div>
                        {form.receiptShowVat!==false&&<div style={{ display: 'flex', justifyContent: 'space-between', fontSize: `${(form.receiptFontSize||11)-1}px` }}><span>ভ্যাট (১৫%):</span><span>৳26.25</span></div>}
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', borderTop: '1px dashed #000', marginTop: 4, paddingTop: 4 }}><span>মোট:</span><span>৳201.25</span></div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: `${(form.receiptFontSize||11)-1}px` }}><span>পরিশোধ:</span><span>৳210</span></div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: `${(form.receiptFontSize||11)-1}px` }}><span>ফেরত:</span><span>৳8.75</span></div>
                      </div>
                      {form.receiptShowQr!==false&&<div style={{ textAlign: 'center', marginTop: 8 }}><div style={{ width: 50, height: 50, margin: '0 auto 4px', background: 'repeating-linear-gradient(0deg,#000 0px,#000 1px,#fff 1px,#fff 3px),repeating-linear-gradient(90deg,#000 0px,#000 1px,#fff 1px,#fff 3px)', backgroundSize: '3px 3px,3px 3px', border: '1px solid #000', borderRadius: 2 }}></div><div style={{ fontSize: 7, color: '#000' }}>ZATCA QR</div></div>}
                      <div style={{ textAlign: 'center', borderTop: '1px dashed #000', marginTop: 8, paddingTop: 6, fontSize: `${(form.receiptFontSize||11)-2}px` }}>{form.receiptFooter||'ধন্যবাদ'}<br/>২ আগস্ট, ২০২৬</div>
                    </div>
                  </div>
                  ...
                  <div style={{ background: '#f1f5f9', borderRadius: 12, padding: 16 }}>
                    ...
                    <div style={{ background: '#fff', padding: `${form.purchaseFontSize||11}px`, width: 220, margin: '0 auto', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', fontSize: `${form.purchaseFontSize||11}px`, fontFamily: "'Tiro Bangla','Courier New',monospace", color: '#000' }}>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: `${(form.purchaseFontSize||11)-1}px` }}>
                        <thead><tr style={{ borderBottom: '1px dashed #000' }}><th style={{ textAlign: 'left', padding: '3px 0' }}>পণ্য</th><th style={{ textAlign: 'center', padding: '3px 0' }}>পরি</th><th style={{ textAlign: 'right', padding: '3px 0' }}>দাম</th><th style={{ textAlign: 'right', padding: '3px 0' }}>মোট</th></tr></thead>
                        <tbody>
                          <tr><td style={{ padding: '3px 0' }}>পণ্য নাম ১<br/><span style={{ fontSize: `${(form.purchaseFontSize||11)-3}px`, color: '#666' }}>কোম্পানি</span></td><td style={{ textAlign: 'center', padding: '3px 0' }}>10</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳50</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳500</td></tr>
                          <tr><td style={{ padding: '3px 0' }}>পণ্য নাম ২</td><td style={{ textAlign: 'center', padding: '3px 0' }}>5</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳80</td><td style={{ textAlign: 'right', padding: '3px 0' }}>৳400</td></tr>
                        </tbody>
                      </table>
                      <div style={{ borderTop: '1px dashed #000', marginTop: 6, paddingTop: 6 }}>
            ...
            <div style={{
              ...
              {users.length === 0 ? (
                <div style={{ textAlign: 'center', padding: 40, color: '#64748b' }}>
                  <p style={{ fontSize: 18 }}>কোনো ইউজার নেই। ➕ বাটনে ক্লিক করে নতুন ইউজার যোগ করুন।</p>
                </div>
              ) : users.map(u => (
                <div key={u.id} style={{
                  padding: 20,
        ...
        {activeTab === 3 && (
          <div>
            <div style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                {[
                  { l: '📦 পণ্য ডেটা', c: products.length, btn: 'সব পণ্য মুছে ফেলুন', fn: async () => { if(confirm('সব পণ্য মুছে ফেলবেন?')) { for(const p of products) { await productsApi.delete(p.id); } await refreshData(); alert('পণ্য মুছা হয়েছে।'); } } },
                  ...
                  { l: '📂 ক্যাটাগরি ডেটা', c: categories.length, btn: 'সব ক্যাটাগরি মুছে ফেলুন', fn: async () => { if(confirm('সব ক্যাটাগরি মুছে ফেলবেন?')) { for(const c of categories) { await categoriesApi.delete(c.id); } await refreshData(); alert('ক্যাটাগরি মুছা হয়েছে।'); } } },
                  { l: '🛒 বিক্রয় ডেটা', c: sales.length, btn: 'সব বিক্রয় মুছে ফেলুন', fn: async () => { if(confirm('সব বিক্রয় মুছে ফেলবেন?')) { for(const s of sales) { await salesApi.delete(s.id); } await refreshData(); alert('বিক্রয় মুছা হয়েছে।'); } } },
                  { l: '👥 কাস্টমার ডেটা', c: customers.length, btn: 'সব কাস্টমার মুছে ফেলুন', fn: async () => { if(confirm('সব কাস্টমার মুছে ফেলবেন?')) { for(const c of customers) { await customersApi.delete(c.id); } await refreshData(); alert('কাস্টমার মুছা হয়েছে।'); } } },
                  ...
                  { l: '👤 সকল ইউজার (সুপার এডমিন ছাড়া)', c: users.filter(u => u.role !== 'super_admin').length, btn: 'সব ইউজার মুছে ফেলুন', fn: async () => { if(confirm('সুপার এডমিন ছাড়া সব ইউজার মুছে ফেলবেন?')) { for(const u of users.filter(u => u.role !== 'super_admin')) { await usersApi.delete(u.id); } const userList = await usersApi.getAll(); setUsers(userList || []); alert('সব ইউজার মুছা হয়েছে।'); } } },
                ].map((d, i) => (
                  <div key={i} style={{
                    padding: 24,
                    background: '#fef2f2',
                ...
                <button
                  onClick={async () => {
                    if(confirm('⚠️ সত্যিই সব ডেটা মুছে ফেলবেন? এটি পূর্বাবস্থায় ফেরানো যাবে না।')) {
                      ...
                      for(const p of products) { await productsApi.delete(p.id); }
                      for(const c of customers) { await customersApi.delete(c.id); }
                      for(const s of suppliers) { await suppliersApi.delete(s.id); }
                      for(const cat of categories) { await categoriesApi.delete(cat.id); }
                      for(const s of sales) { await salesApi.delete(s.id); }
                      for(const p of purchases) { await purchasesApi.delete(p.id); }
                      for(const u of users.filter(u => u.role !== 'super_admin')) { await usersApi.delete(u.id); }
