    ...
    return (
      <div style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'10px 12px',display:'flex',gap:8,alignItems:'center',background:T.white,borderBottom:`1px solid ${T.gray200}`}}>
          ...
          <button style={{...btn('primary'),marginLeft:'auto',padding:'6px 12px',fontSize:12}} onClick={()=>{const html=`<!DOCTYPE html><html><head><link href="https://fonts.googleapis.com/css2?family=Tiro+Bangla&display=swap" rel="stylesheet"><title>${viewSupplier.name} - সরবরাহকারী</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Tiro Bangla',Arial,sans-serif;padding:30px;font-size:13px;color:#333}@page{size:A4;margin:20mm}.header{border-bottom:2px solid #333;padding-bottom:16px;margin-bottom:24px}.header h1{font-size:26px;font-weight:700;margin-bottom:4px}.header p{color:#666;font-size:12px}.row{display:flex;gap:40px;margin-bottom:12px}.label{color:#888;width:100px}.value{font-weight:600}.divider{border-top:1px solid #eee;margin:20px 0}.stats{display:flex;gap:30px;margin:20px 0}.stat{text-align:center}.stat .num{font-size:28px;font-weight:700}.stat .label{font-size:11px;color:#888;margin-top:4px}.total{background:#f5f5f5;padding:16px 20px;border-radius:8px;margin-top:20px;text-align:right}.total .label{font-size:11px;color:#666}.total .amount{font-size:24px;font-weight:700;margin-top:4px}.footer{margin-top:40px;padding-top:16px;border-top:1px solid #eee;text-align:center;color:#999;font-size:11px}</style></head><body><div class="header"><h1>🏢 ${viewSupplier.name}</h1><p>সরবরাহকারী প্রোফাইল</p></div>${viewSupplier.phone?`<div class="row"><span class="label">ফোন:</span><span class="value">${viewSupplier.phone}</span></div>`:''}${viewSupplier.crNumber?`<div class="row"><span class="label">CR:</span><span class="value">${viewSupplier.crNumber}</span></div>`:''}${viewSupplier.vatNumber?`<div class="row"><span class="label">VAT:</span><span class="value">${viewSupplier.vatNumber}</span></div>`:''}${viewSupplier.address?`<div class="row"><span class="label">ঠিকানা:</span><span class="value">${viewSupplier.address}</span></div>`:''}<div class="divider"></div><div class="stats"><div class="stat"><div class="num">${supProducts.length}</div><div class="label">পণ্য</div></div><div class="stat"><div class="num">${supPurchases.length}</div><div class="label">পারচেজ</div></div><div class="stat"><div class="num">${supPurchases.reduce((s,p)=>s+p.totalStock,0)}</div><div class="label">একক</div></div></div>${totalPurchase>0?`<div class="total"><div class="label">মোট (VAT সহ)</div><div class="amount">৳${totalPurchase.toLocaleString('en-US',{minimumFractionDigits:2})}</div></div>`:''}<div class="footer">প্রিন্ট তারিখ: ${new Date().toLocaleString('bn-BD')}</div></body></html>`;const w=window.open('','_blank');w.document.write(html);w.document.close();w.onload=()=>setTimeout(()=>w.print(),100);}}>🖨️ প্রিন্ট</button>
        </div>
