        ...
        {activeTab === 'categories' && (
          <div>
            <div style={{...card,marginBottom:12,padding:12,display:'flex',justifyContent:'space-between',alignItems:'center',background:T.white,borderRadius:10}}>
              ...
              <button style={{...btn('primary'),padding:'6px 12px',fontSize:12}} onClick={()=>{const html=`<!DOCTYPE html><html><head><link href="https://fonts.googleapis.com/css2?family=Tiro+Bangla&display=swap" rel="stylesheet"><title>ক্যাটাগরি লিস্ট</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Tiro Bangla',Arial,sans-serif;padding:15px;font-size:11px}@page{size:A4 landscape;margin:10mm}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:6px 8px}th{background:#e0f7f0;font-weight:700;color:#00897b}.header{text-align:center;border-bottom:2px solid #00897b;padding-bottom:10px;margin-bottom:15px}.header h1{color:#00897b;font-size:18px}.center{text-align:center}tr:nth-child(even){background:#f9f9f9}.total{background:#e0f7f0;font-weight:700}.total td{text-align:center}</style></head><body><div class="header"><h1>📂 ক্যাটাগরি লিস্ট</h1></div><table><thead><tr><th class="center" style="width:8%">ক্রম</th><th style="width:72%">ক্যাটাগরির নাম</th><th class="center" style="width:20%">পণ্য সংখ্যা</th></tr></thead><tbody>${categories.map((c,i)=>`<tr><td class="center">${i+1}</td><td>${c.name}</td><td class="center">${products.filter(p=>p.cat===c.name).length}টি</td></tr>`).join('')}</tbody><tfoot><tr class="total"><td colspan="3">মোট ক্যাটাগরি: ${categories.length}টি | পণ্য: ${products.filter(p=>p.cat&&p.name).length}টি</td></tr></tfoot></table><p style="margin-top:15px;text-align:center;color:#999;font-size:10px">প্রিন্ট তারিখ: ${new Date().toLocaleString('bn-BD')}</p></body></html>`;const w=window.open('','_blank','width=1200,height=800');w.document.write(html);w.document.close();w.onload=()=>setTimeout(()=>w.print(),100);}}>🖨️ প্রিন্ট</button>
            </div>
