...
সুজি বিস্কুট,C-00002,সুজান,স্ন্যাকস,004,পিস,15,20,150,40`;
    ...
    return (
      <div style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'10px 12px',display:'flex',gap:8,alignItems:'center',background:T.white,borderBottom:`1px solid ${T.gray200}`}}>
          ...
          <button style={{...btn('primary'),padding:'6px 12px',fontSize:12}} onClick={()=>{const tot=s=>s.reduce((a,i)=>a+(i.stock||0)*(i.buyP||0),0)*1.15;const grandTotal=supplierPurchases.reduce((s,p)=>s+tot(p.items),0);const totalItems=supplierPurchases.reduce((s,p)=>s+p.totalItems,0);const totalStock=supplierPurchases.reduce((s,p)=>s+p.totalStock,0);const html=`<!DOCTYPE html><html><head><link href="https://fonts.googleapis.com/css2?family=Tiro+Bangla&display=swap" rel="stylesheet"><title>পারচেজ হিস্ট্রি</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Tiro Bangla',Arial,sans-serif;padding:20px;font-size:12px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background:#e0f7f0;font-weight:700;color:#00897b}.total{background:#e0f7f0;font-weight:700}</style></head><body><h2>📦 ${showPurchaseHistory.name} - পারচেজ হিস্ট্রি</h2><br/><table><thead><tr><th>তারিখ</th><th>আইডি</th><th style="text-align:center">পণ্য</th><th style="text-align:center">একক</th><th>মোট</th></tr></thead><tbody>${[...supplierPurchases].reverse().map(p=>`<tr><td>${new Date(p.date).toLocaleDateString('bn-BD')}</td><td>${p.id}</td><td style="text-align:center">${p.totalItems}টি</td><td style="text-align:center">${p.totalStock}</td><td>৳${tot(p.items).toLocaleString('en-US',{minimumFractionDigits:2})}</td></tr>`).join('')}</tbody><tfoot><tr class="total"><td colspan="1" style="text-align:right">মোট:</td><td style="text-align:center">পারচেজ: ${supplierPurchases.length}টি</td><td style="text-align:center">পণ্য: ${totalItems}টি</td><td style="text-align:center">একক: ${totalStock}টি</td><td>৳${grandTotal.toLocaleString('en-US',{minimumFractionDigits:2})}</td></tr></tfoot></table><p style="margin-top:20px">প্রিন্ট তারিখ: ${new Date().toLocaleString('bn-BD')}</p></body></html>`;const w=window.open('','_blank');w.document.write(html);w.document.close();w.onload=()=>setTimeout(()=>w.print(),100);}}>🖨️ প্রিন্ট</button>
        </div>
        <div style={{flex:1,overflow:'auto',padding:12}}>
    ...
    return (
      <div style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'10px 12px',display:'flex',gap:8,alignItems:'center',background:T.white,borderBottom:`1px solid ${T.gray200}`}}>
          ...
          <button style={{...btn('primary'),marginLeft:'auto',padding:'6px 12px',fontSize:12}} onClick={()=>{const html=`<!DOCTYPE html><html><head><link href="https://fonts.googleapis.com/css2?family=Tiro+Bangla&display=swap" rel="stylesheet"><title>${viewSupplier.name} - সরবরাহকারী</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Tiro Bangla',Arial,sans-serif;padding:30px;font-size:13px}@page{size:A4;margin:20mm}body{background:#fff}.card{background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.08);margin-bottom:20px;overflow:hidden}.card-header{background:linear-gradient(135deg,#00897b 0%,#00695c 100%);color:white;padding:20px 24px}.card-header h1{margin:0;font-size:28px;font-weight:700}.card-header p{margin:6px 0 0;opacity:0.85;font-size:13px}.card-body{padding:24px}.info-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:16px}.info-item{background:#f8faf9;border-radius:8px;padding:14px 16px;border-left:4px solid #00897b}.info-item .label{font-size:11px;color:#888;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px}.info-item .value{font-size:15px;font-weight:700;color:#333}.vat-card{background:linear-gradient(135deg,#059669 0%,#047857 100%);color:white;border-radius:12px;padding:20px 24px;margin:20px 0;display:flex;justify-content:space-between;align-items:center}.vat-card .vat-info .vat-label{font-size:11px;opacity:0.85;margin-bottom:4px}.vat-card .vat-info .vat-number{font-size:22px;font-weight:800;letter-spacing:2px}.vat-card .vat-icon{font-size:40px;opacity:0.3}.stats-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin:20px 0}.stat-card{background:#f8faf9;border-radius:12px;padding:20px;text-align:center;border-top:4px solid #00897b}.stat-card.green{border-top-color:#059669}.stat-card.orange{border-top-color:#e65100}.stat-card .stat-num{font-size:36px;font-weight:800;color:#00897b;line-height:1}.stat-card.green .stat-num{color:#059669}.stat-card.orange .stat-num{color:#e65100}.stat-card .stat-label{font-size:12px;color:#666;margin-top:8px;text-transform:uppercase;letter-spacing:0.5px}.total-section{background:linear-gradient(135deg,#e0f7f0 0%,#b2dfdb 100%);border-radius:12px;padding:20px 24px;margin-top:20px;text-align:right;border:2px solid #00897b}.total-section .total-label{font-size:12px;color:#666;text-transform:uppercase;letter-spacing:0.5px}.total-section .total-amount{font-size:32px;font-weight:800;color:#00897b;margin-top:4px}.footer{margin-top:30px;padding-top:16px;border-top:1px solid #eee;text-align:center;color:#999;font-size:11px}</style></head><body><div class="card"><div class="card-header"><h1>🏢 ${viewSupplier.name}</h1><p>সরবরাহকারী প্রোফাইল</p></div><div class="card-body"><div class="info-grid">${viewSupplier.phone?`<div class="info-item"><div class="label">📞 ফোন নম্বর</div><div class="value">${viewSupplier.phone}</div></div>`:''}${viewSupplier.crNumber?`<div class="info-item"><div class="label">🏢 CR নম্বর</div><div class="value">${viewSupplier.crNumber}</div></div>`:''}</div>${viewSupplier.address?`<div class="info-item" style="grid-column:1/-1"><div class="label">📍 ঠিকানা</div><div class="value">${viewSupplier.address}</div></div>`:''}${viewSupplier.vatNumber?`<div class="vat-card"><div class="vat-info"><div class="vat-label">🧾 VAT নম্বর (ZATCA ক্রেডিট)</div><div class="vat-number">${viewSupplier.vatNumber}</div></div><div class="vat-icon">🧾</div></div>`:''}<div class="stats-grid"><div class="stat-card"><div class="stat-num">${supProducts.length}</div><div class="stat-label">মোট পণ্য</div></div><div class="stat-card green"><div class="stat-num">${supPurchases.length}</div><div class="stat-label">পারচেজ সংখ্যা</div></div><div class="stat-card orange"><div class="stat-num">${supPurchases.reduce((s,p)=>s+p.totalStock,0)}</div><div class="stat-label">মোট একক</div></div></div>${totalPurchase>0?`<div class="total-section"><div class="total-label">মোট পারচেজ এমাউন্ট (VAT সহ)</div><div class="total-amount">৳${totalPurchase.toLocaleString('en-US',{minimumFractionDigits:2})}</div></div>`:''}</div></div><div class="footer">প্রিন্ট তারিখ: ${new Date().toLocaleString('bn-BD')} | ${viewSupplier.name} - সরবরাহকারী প্রোফাইল</div></body></html>`;const w=window.open('','_blank');w.document.write(html);w.document.close();w.onload=()=>setTimeout(()=>w.print(),100);}}>🖨️ প্রিন্ট</button>
        </div>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      {/* COMPANIES TAB */}
        {activeTab === 'companies' && (
          <div style={{...card,marginBottom:16,padding:12,display:'flex',justifyContent:'space-between',alignItems:'center',background:T.white,borderRadius:10}}>
