...
function ReportsScreen({sales, customers, purchases, settings}) {
  ...
  const printPurchases = () => {
    ...
    const totalVatReceived = fs.reduce((s,sale)=>s+sale.total*0.15,0);
    ...
    fs.forEach(s => {
      html += `
          <tr>
            ...
            <td style="text-align:right;color:#00897b;">৳${(s.total*0.15).toLocaleString('en-US', {minimumFractionDigits: 2})}</td>
          </tr>`;
    });
        ...
        <div style={card}>
          ...
          <div style={{overflow:'auto'}}>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead>
                <tr style={{background:T.gray50}}>
                  {['তারিখ','বিল নং','কাস্টমার','পণ্য','মোট','পরিশোধ','বাকি','লাভ','ভ্যাট'].map(h=>(
                    <th key={h} style={{padding:'8px 10px',textAlign:'left',fontSize:11,fontWeight:700,color:T.gray400,borderBottom:`1px solid ${T.gray200}`,whiteSpace:'nowrap'}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {fs.length===0 ? <tr><td colSpan={9} style={{padding:30,textAlign:'center',color:T.gray400}}>নির্বাচিত সময়ে কোনো বিক্রয় নেই</td></tr>
                : [...fs].reverse().map((s,i)=>(
                  <tr key={s.id} style={{background:i%2===0?T.white:'#FAFAFA',borderBottom:`1px solid ${T.gray100}`}}>
              ...
              <tfoot>
                <tr style={{background:T.greenLight}}>
                  ...
                  <td style={{padding:'10px',fontWeight:700,fontSize:13,color:'#00897b'}}>{fmt(fs.reduce((s,sale)=>s+sale.total*0.15,0))}</td>
                </tr>
              </tfoot>
            </table>
          </div>
