    ...
    return (
      <div style={{height:'100%',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'10px 12px',display:'flex',gap:8,alignItems:'center',background:T.white,borderBottom:`1px solid ${T.gray200}`}}>
          ...
          <button style={{...btn('primary'),padding:'6px 12px',fontSize:12}} onClick={()=>{const tot=s=>s.reduce((a,i)=>a+(i.stock||0)*(i.buyP||0),0)*1.15;const grandTotal=supplierPurchases.reduce((s,p)=>s+tot(p.items),0);const html=`<!DOCTYPE html><html><head><link href="https://fonts.googleapis.com/css2?family=Tiro+Bangla&display=swap" rel="stylesheet"><title>পারচেজ হিস্ট্রি</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Tiro Bangla',Arial,sans-serif;padding:20px;font-size:12px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background:#e0f7f0;font-weight:700;color:#00897b}.total{background:#e0f7f0;font-weight:700}</style></head><body><h2>📦 ${showPurchaseHistory.name} - পারচেজ হিস্ট্রি</h2><p>মোট পারচেজ: ${supplierPurchases.length}টি</p><br/><table><thead><tr><th>তারিখ</th><th>আইডি</th><th>পণ্য</th><th>একক</th><th>মোট</th></tr></thead><tbody>${[...supplierPurchases].reverse().map(p=>`<tr><td>${new Date(p.date).toLocaleDateString('bn-BD')}</td><td>${p.id}</td><td>${p.totalItems}টি</td><td>${p.totalStock}</td><td>৳${tot(p.items).toLocaleString('en-US',{minimumFractionDigits:2})}</td></tr>`).join('')}</tbody><tfoot><tr class="total"><td colspan="4">মোট:</td><td>৳${grandTotal.toLocaleString('en-US',{minimumFractionDigits:2})}</td></tr></tfoot></table><p style="margin-top:20px">প্রিন্ট তারিখ: ${new Date().toLocaleString('bn-BD')}</p></body></html>`;const w=window.open('','_blank');w.document.write(html);w.document.close();w.onload=()=>setTimeout(()=>w.print(),100);}}>🖨️ প্রিন্ট</button>
        </div>
        <div style={{flex:1,overflow:'auto',padding:12}}>
